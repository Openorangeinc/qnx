...
//sig handler
//signal receiver
sig_atomic_t usr1Happened =0; //no interrupt when being read/written on
...
void* thread_factory(void* p){
	...
	while(true){
		...
		if(sem_id!=SUCCESS){ //success ==int 0
			//if not success
			perror("\nsem_wait was not successful\n");
		}

		//unblock the thread
		printf("child thread with pid %d unblocked",pthread_self());
		//wait 5 sec
		sleep(5);
	}
	return NULL;
...
int main(void) {
	struct sigaction Sigusr1;
		...
		//closing sem_t
		 ...
		 if(sem_unlink('pSem_t') ==-1) perror("sem_close returned -1");
